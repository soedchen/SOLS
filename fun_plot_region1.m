function [mp,co] = fun_plot_region(lon,lat,data,titles,colorbar_status)
% 绘图
% load("05021258.mat")
...
%data = log10(data);
m_proj('lambert','lon',[105 140],'lat',[10 45]);
% mp = m_pcolor(LG,LT,data);
mp = m_pcolor(lon,lat,data');
...
color = m_colmap('jet','step',120);
cf = color(1:60,:,:);
cl = color(60:120,:,:);
cff = flipud(m_colmap('water',40));
colormap([cff(1:30,:,:);m_colmap('jet',90)]); 
% colormap([m_colmap(cf,30) m_colmap(cl,90)]);
...
%     set(co,'tickdir','in');
    ...
    h = co;
%     lab= [0.1 5 10 15 20 25 30 50 70 90 110];
%   set(h,'ytick',log10(lab),'yticklabel',lab,...
%         'tickdir','out');
end
% pos = get(h,'pos');
