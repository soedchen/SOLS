clear;close all;clc
% Simulation with changing depth
addpath(genpath(pwd));
...
for i =1:length(Zs)
    ...
    mm=1;
    Z = Zs(i);
    [m_pc_single,z_pc_single,Prb,z,t,In,Nz] = fun_PC(E0,A,FOV,O,To,Ta, ...
        Ts,eta,n,H,v,delta_t,theta,delta,delta_lambda,Z,lambda,chl,Rb,W,Nd,deadtime,mm,day);
    m_pc = [m_pc m_pc_single*i];
    z_pc = [z_pc z_pc_single];
end
% figure;
...
subplot(222)
dh = 0.1;
h = min(z_pc):dh:max(z_pc);
N = zeros(size(h));
for i=1:length(h)
    N(i)=sum((z_pc>(h(i)-dh/2) & z_pc<(h(i))+dh/2));
end
plt=semilogx(N,-h);ylim([-40 5])
xlabel("Number of photons");ylabel("Depth (m)");xlim([0 1e3])
ax = plt.Parent;
