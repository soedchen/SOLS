function [hh,h] = plotGlobe_fun_range(lon,lat,data,titles,colorbar_status,mi,ma)
% 绘图,在原有的基础上增加最大最小值
% load("05021258.mat")
...
caxis([mi,ma]);
if colorbar_status
    h=colorbar('southoutside'); % 'north' | 'south' | 'east' | 'west' | 'northoutside'
    % 'pos', [横坐标，纵坐标，宽度，高度] 位置
    % 'ticks',刻度线位置
    set(h,'tickdir','in');
    % set('Fontsize',14)
    % 参加文字标识
    h.Label.String = titles;
end
% pos = get(h,'pos');
